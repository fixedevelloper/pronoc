...
export default function MyPotsPage() {
    ...
    const fetchPots = async () => {
        ...
        setLoading(true);
        try {
            const res = await axiosServices.get("/api/account/pots", {
        ...
        <div className="max-w-8xl mx-auto mt-8 px-4 md:px-6 grid grid-cols-1 md:grid-cols-3 gap-6">
           ...
           <AccountNavigation />
            ...
            <div className="md:col-span-2">
                <h1 className="text-2xl font-bold text-blue-600 mb-4">Mes Pots</h1>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {pots.map((pot) => (
                            <div key={pot.id} className="p-5 bg-white dark:bg-gray-800 rounded-lg shadow flex flex-col justify-between hover:shadow-lg transition">
                                ...
                                <Link href={`/account/pots/${pot.id}`} className="mt-4 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-500 transition text-center">
                                    Voir détails
                                </Link>
                                <Link href={`/pots/${pot.id}/view`} className="mt-4 px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-black-500 transition text-center">
                                    Inviter les amis
                                </Link>
                                </div>
                            </div>
